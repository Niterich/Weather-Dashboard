...
function requestWeather(cityName) {
  ...
  queryURL = "http://api.openweathermap.org/data/2.5/weather?q=" + cityName + ",us&mode=json&units=imperial&appid=88ac7af34cb90d87533bbf879a2bb928";
  ...
  $.ajax({
    ...
    $.ajax({
      url: "http://api.openweathermap.org/data/2.5/forecast?q=" + cityName + ",us&mode=json&units=imperial&appid=88ac7af34cb90d87533bbf879a2bb928",
      method: "GET"
      ...
      for (let i = 0; i < resThree.list.length; i++) {
        ...
        if (forecastDay.dt_txt.indexOf("18:00") !== -1){
          ...
          const forecastImage = $("<img>").attr("src", "http://openweathermap.org/img/wn/" + forecastDay.weather[0].icon + "@2x.png").appendTo(dayCard);
